...
```{python}
# Import packages needed in this document
...
A full index tuple leads to a specific column, which is a time series of open/high/low/close prices or volumes for a stock. We can extract it as a Series:
...
nvda_close
```
It is straightfoward to generate a daily return series and time series line plots with it: 

```{python}
#| warning: false
#| label: fig-nvda-close
#| fig-cap: "Daily Close Prices and Returns of NVDA in 2024"

fig, (ax1, ax2) = plt.subplots(2, sharex=True)
ax1.plot(nvda_close)
ax2.plot(nvda_close.pct_change())
```

# Accessing Multiple Columns
More frequently, we need to extract multiple columns from the above multindex DataFrame to generate a new DataFrame. For example, we need a data set including only daily close prices of both NVDA and TSLA in 2024.

Of course we can use the complete list of column index tuples to refer to these columns:

```{python}
#| warning: false
#| label: tbl-close
#| tbl-cap: "Daily Close Prices of NVDA and TSLA in 2024"

close = df1.loc[:, [('Close', 'NVDA'), ('Close', 'TSLA')]]
